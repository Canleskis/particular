...
impl<P: Particle> ParticleSet<P> {
    ...
    /// Creates an empty [`ParticleSet`].
    #[cfg(feature = "gpu")]
    pub fn new(particle_count: usize, device: &wgpu::Device) -> Self {
        Self {
            massive: Vec::new(),
            massless: Vec::new(),

            wgpu: crate::gpu::Wgpu::init(particle_count, device),
        }
    }
}
...
impl<P: Particle> ParticleSet<P> {
    ...
    #[cfg(feature = "gpu")]
    fn compute_gpu(&self, device: &wgpu::Device, queue: &wgpu::Queue) -> Vec<SIMD> {
        let massive = self.massive_particles();
        let massless = self.massless_particles();

        self.wgpu
            .update(massive.into_iter().chain(massless.into_iter()).collect(), device, queue)
            .into_iter()
            .map(SIMD::from)
            .collect()
    }
    ...
    #[cfg(feature = "gpu")]
    pub fn accelerations_gpu(&self, device: &wgpu::Device, queue: &wgpu::Queue) -> impl Iterator<Item = <P::Vector as Descriptor>::Type> {
        self.compute_gpu(device, queue).into_iter().map(P::Vector::from_simd)
    }
    ...
    ) -> impl Iterator<Item = (<P::Vector as Descriptor>::Type, &mut P)> {
        self.accelerations_gpu(device, queue).zip(self.iter_mut())
    }
}
